...
def visual_network(save_data=False, nb_round=500, time_window=pow(10,2), data="classic",draw=True,test=False):
  ...
  def experiment(trail_nb=nb_round,print_keeps=False,keep_flags=None,thr=0.5,draw=draw,deep_keeps=False,count=False,**kwargs):
    ...
    for i in xrange(trail_nb):
      ...
      n.impose_current_to_group(("activator %s" % (neuron_set)), current_function = Current(1,**{"intensity":10,"random":True}))
  ...
  def learning(draw=draw):
    ...
    for i in xrange(int(4*nb_round)):
      neuron_set = randint(0,nb_group-1)
      n.impose_current_to_group(("activator %s" % (neuron_set)), current_function = Current(1,**{"frequency":10}))
      n.run()
      n.clean_current()
